    ...
    venta.save((err,venta_save)=>{
        if (venta_save) {
            ...
            detalles.forEach((element,index)=>{
                ...
                detalleventa.precio_FN = venta_save.precio_FN;
                ...
                detalleventa.save((err, detalle_save)=>{
                    if (detalle_save) {
                        Producto.findById({_id:element.idproducto},(err,producto_data)=>{
                            if (producto_data) {
                                Producto.findByIdAndUpdate({_id:producto_data._id},{stock: 
                                    parseInt(producto_data.stock) -parseInt(element.cantidad)},(err,producto_edit)=>{
                                    res.end();
                                })
                            } else {
                        ...
                        res.send('No se registro los datos');
                    }
                });
            });
        } else {
...
function datos_venta(req,res){ 
    ...
    Venta.findById(id).populate('idcliente').populate('iduser').exec((err,data_venta)=>{
        if(data_venta){
            ...
            DetalleVenta.find().populate('idproducto').exec({idventa:id},(err,data_detalle)=>{
                if(data_detalle){
