    ...
    Producto.find({titulo: new RegExp(titulo,'i')}).populate('idcategoria').exec((err, productos_listado)=>{
        if (err) {
            ...
            if (productos_listado) {
                ...
                res.status(403).send({message:'No hay ningun resgistro con ese titulo'});
            }
        }
    });
}
...
function editar(req, res){
    ...
    if (req.files.imagen) {
        //bug sin resolved I need spring 3
        ...
        puntos: data.puntos}, (err, producto_edit)=>{
            if (err) {
                ...
                if (producto_edit) {
                    res.status(200).send({producto: producto_edit});          //producto
                } else {
        ...
        Producto.findByIdAndUpdate({_id:id},{titulo: data.titulo, descripcion: data.descripcion,
            ...
            puntos: data.puntos}, (err, producto_edit)=>{
                if (err) {
                    ...
                    if (producto_edit) {
                        res.status(200).send({producto: producto_edit});          //produnto
                    } else {
                        res.status(403).send({message: 'No se edito el producto'});
                    }
        ...
        if (err) {
            ...
            if (producto_delete) {
               ...
               res.status(200).send({producto: producto_delete}); 
                ...
                res.status(403).send({message: 'No se elimino ningun Producto'});
            }
        }
    });
}
